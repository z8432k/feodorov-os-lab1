...
Строки в файле отсортировать в порядке убывания числа запросов к доменам DNS-туннелей. Пользователей с одинаковым количеством запросов дополнительно отсортировать по возрастанию IP-адреса, интерпретируя IP-адрес как строку, а не как набор числовых полей. Это значит, что `192.168.0.100` будет идти раньше, чем `192.168.0.2`.
